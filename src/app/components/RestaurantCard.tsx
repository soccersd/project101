...
export default function RestaurantCard({ data, className = "", onClick }: RestaurantProps) {


    return (
        <div
            onClick={onClick}
            className={`group relative bg-white rounded-2xl overflow-hidden border border-zinc-100/80 shadow-sm transition-all duration-500 ease-out hover:shadow-2xl hover:shadow-orange-500/8 hover:-translate-y-2 hover:border-orange-100 flex flex-col h-full cursor-pointer ${className}`}
        >
            {/* Image Container */}
            ...
            <div className="p-5 lg:p-6 flex flex-col flex-grow relative bg-white">
                ...
                {/* Info (Hours & Tel) */}
                {(data.hours || data.tel) && (
                    <div className="flex flex-col gap-1.5 mb-3 text-xs text-zinc-500 bg-zinc-50 p-2 rounded-lg border border-zinc-100">
                        {data.hours && (
                            <div className="flex items-start gap-2">
                                <svg className="w-3.5 h-3.5 text-zinc-400 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        {data.tel && (
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-mono">{data.tel}</span>
                            </div>
                        )}
                    </div>
                )}
